...
use system\lib\guzzle\Cookie\CookieJarInterface;
        ...
        return function (callable $handler): callable {
            return function ($request, array $options) use ($handler) {
                if (empty($options['cookies'])) {
                    ...
                    throw new \InvalidArgumentException('cookies must be an instance of system\lib\guzzle\Cookie\CookieJarInterface');
                }
            ...
            return function (RequestInterface $request, array $options) use ($handler, &$container) {
                return $handler($request, $options)->then(
                    ...
                    function ($reason) use ($request, &$container, $options) {
                        ...
                        return \system\lib\guzzle\promises\rejection_for($reason);
                    }
                );
            };
        };
                ...
                return $handler($request, $options)->then(
                    ...
                    function ($reason) use ($logger, $request, $formatter): PromiseInterface {
                        $response = $reason instanceof RequestException
                            ? $reason->getResponse()
                            : null;
                        $message = $formatter->format($request, $response, \system\lib\guzzle\promises\exception_for($reason));
